...
private:
  ...
  // const edm::EDGetTokenT<TTTrackCollection> l1TracksToken_; //FIXME: Remove
  ...
  // const double ptMin_, absEtaMax_, absZ0Max_, bendChi2Max_, reducedChi2RZMax_, reducedChi2RPhiMax_;  //FIXME: Remove
  // const int nStubsMin_, nPSStubsMin_;  //FIXME: Remove
...
L1TrackVertexAssociationProducer::L1TrackVertexAssociationProducer(const edm::ParameterSet& iConfig)
    : // l1SelectedTracksToken_(consumes<TTTrackCollection>(iConfig.getParameter<edm::InputTag>("l1SelectedTracksInputTag"))), //FIXME: REMOVE
      // l1SelectedTracksEmulationToken_(consumes<TTTrackCollection>(iConfig.getParameter<edm::InputTag>("l1SelectedTracksEmulationInputTag"))), //FIXME: Remove
      tTopoToken_(esConsumes<TrackerTopology, TrackerTopologyRcd>(edm::ESInputTag("", ""))),
      outputCollectionName_(iConfig.getParameter<std::string>("outputCollectionName")),
      cutSet_(iConfig.getParameter<edm::ParameterSet>("cutSet")),

      // ptMin_(cutSet_.getParameter<double>("ptMin")), //FIXME:REMOVE
      // absEtaMax_(cutSet_.getParameter<double>("absEtaMax")), //FIXME:REMOVE
      // absZ0Max_(cutSet_.getParameter<double>("absZ0Max")), //FIXME:REMOVE
      // bendChi2Max_(cutSet_.getParameter<double>("reducedBendChi2Max")), //FIXME:REMOVE
      // reducedChi2RZMax_(cutSet_.getParameter<double>("reducedChi2RZMax")), //FIXME:REMOVE
      // reducedChi2RPhiMax_(cutSet_.getParameter<double>("reducedChi2RPhiMax")), //FIXME:REMOVE
      // nStubsMin_(cutSet_.getParameter<int>("nStubsMin")), //FIXME:REMOVE
      // nPSStubsMin_(cutSet_.getParameter<int>("nPSStubsMin")), //FIXME:REMOVE
  ...
  if (processSimulatedTracks_) {
    ...
    // produces<TTTrackRefCollection>(outputCollectionName_); //FIXME: Remove
    if (iConfig.exists("l1VerticesInputTag")) {
      ...
      // produces<TTTrackRefCollection>(outputCollectionName_ + "Associated"); //FIXME: Remove
  ...
  if (processEmulatedTracks_) {
    ...
    // produces<TTTrackRefCollection>(outputCollectionName_ + "Emulation"); //FIXME: Remove
    if (iConfig.exists("l1VerticesEmulationInputTag")) {
      ...
      // produces<TTTrackRefCollection>(outputCollectionName_ + "AssociatedEmulation"); //FIXME: Remove
...
void L1TrackVertexAssociationProducer::printDebugInfo(const TTTrackCollectionHandle& l1SelectedTracksHandle, 
  ...
  // if (processSimulatedTracks_ && processEmulatedTracks_) { //FIXME: Remove
...
void L1TrackVertexAssociationProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  // iEvent.getByToken(l1SelectedTracksToken_, l1SelectedTracksHandle); //FIXME: Remove
  ...
  if (processEmulatedTracks_) {
    ...
    nOutputEmulationApproximate = l1SelectedTracksHandle->size();
    if (doDeltaZCutEmu_) {
  ...
  // TTTrackNPSStubsMinSelector nPSStubsSel(nPSStubsMin_, tTopo); //FIXME:REMOVE
...
void L1TrackVertexAssociationProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  // desc.add<edm::InputTag>("l1TracksInputTag", edm::InputTag("TTTracksFromTrackletEmulation", "Level1TTTracks")); //FIXME: Remove
  desc.add<edm::InputTag>("l1SelectedTracksInputTag", edm::InputTag("L1TrackSelectionProducer", "Level1TTTracksSelected"));
  desc.add<edm::InputTag>("l1SelectedTracksEmulationInputTag", edm::InputTag("L1TrackSelectionProducer", "Level1TTTracksSelectedEmulation"));
  ...
  desc.add<std::string>("outputCollectionName", "Level1TTTracksSelected");
  {
    ...
    // descCutSet.add<double>("ptMin", 2.0)->setComment("pt must be greater than this value, [GeV]"); //FIXME:REMOVE
