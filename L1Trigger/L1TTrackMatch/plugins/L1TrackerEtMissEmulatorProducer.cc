...
L1TrackerEtMissEmulatorProducer::L1TrackerEtMissEmulatorProducer(const edm::ParameterSet& iConfig)
  ...
  // granularity
  int cosLUTbins = 1 << l1tmetemu::kCosLUTTableSize;
  ...
  for (const auto& track : *L1TTTrackHandle) {
    if (std::find(L1TTTrackAssociatedHandle->begin(), L1TTTrackAssociatedHandle->end(), track) !=
      ...
      if (globalPhi >= phiQuadrants_[0] && globalPhi < phiQuadrants_[1]) {
        ...
        temppx = -((l1tmetemu::Et_t)ptEmulation * cosLUT_[(phiQuadrants_[2] - 1 - globalPhi) >> l1tmetemu::kCosLUTShift]);
        ...
        if (debug_ == 2) {
          edm::LogVerbatim("L1TrackerEtMissEmulatorProducer")
              ...
              << "Emu Phi: " << globalPhi << " Emu Cos(Phi): -" << cosLUT_[(phiQuadrants_[2] - 1 - globalPhi) >> l1tmetemu::kCosLUTShift]
        ...
        if (debug_ == 2) {
          edm::LogVerbatim("L1TrackerEtMissEmulatorProducer")
              ...
              << "Emu Phi: " << globalPhi << " Emu Cos(Phi): -" << cosLUT_[(globalPhi - phiQuadrants_[2]) >> l1tmetemu::kCosLUTShift]
              << " Emu Sin(Phi): -" << cosLUT_[(phiQuadrants_[3] - 1 - globalPhi) >> l1tmetemu::kCosLUTShift] << "\n";
        }

      } else if (globalPhi >= phiQuadrants_[3] && globalPhi < phiQuadrants_[4]) {
        temppx = ((l1tmetemu::Et_t)ptEmulation * cosLUT_[(phiQuadrants_[4] - 1 - globalPhi) >> l1tmetemu::kCosLUTShift]);
        ...
        if (debug_ == 2) {
          edm::LogVerbatim("L1TrackerEtMissEmulatorProducer")
              ...
              << " Emu Phi: " << globalPhi << " Emu Cos(Phi): " << cosLUT_[(phiQuadrants_[4] - 1 - globalPhi) >> l1tmetemu::kCosLUTShift]
