...
Produces, in ONE cmsRun job on a GEN-SIM-DIGI-RAW-MINIAOD input:
  ...
  (2) the SmartPixels digiRefit track + refit-hit + stub tables for the chosen
      ...
      'trackCov' is valid on this covMatrix build).
...
Local test:
  cmsRun runUnifiedNano_cfg.py maxEvents=2 activeSP=1100 seedCovMode=trackCov \
    pixelavAngleSet=/work/spxsmoke/pixelav_angle_example17.json \
    inputFiles=file:/work/testfiles/TT_...numEvent100.root \
    outputFile=/work/spx_fullnano_test.root
"""
import os
...
# re-emul fix (c): some CMSSW_14-era L1 muon products are schema-incompatible with 17
# (l1t::TrackerMuon::muRef_ BXVector<RegionalMuonCand> -> vector<SAMuon>). We re-emulate
# muons in-job, so drop the stale input copies. inputCommands AFTER the source.
process.source.inputCommands = cms.untracked.vstring(
    'keep *',
    'drop l1tTrackerMuons_l1tTkMuonsGmt_*_*',
    'drop l1tTrackerMuons_l1tTkMuonsGmtLowPtFix_*_*',
)
