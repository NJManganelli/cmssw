#include "L1Trigger/VertexFinder/interface/VertexProducerDownstream.h"
...
VertexProducerDownstream::VertexProducerDownstream(const edm::ParameterSet& iConfig)
  ...
  switch (settings_.vx_algo()) {
    case Algorithm::fastHisto:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using the fastHisto binning algorithm";
      break;
    case Algorithm::fastHistoEmulation:
      edm::LogInfo("VertexProducerDownstream")
          << "VertexProducerDownstream::Finding vertices using the emulation version of the fastHisto binning algorithm";
      break;
    case Algorithm::fastHistoLooseAssociation:
      edm::LogInfo("VertexProducerDownstream")
          << "VertexProducerDownstream::Finding vertices using the fastHistoLooseAssociation binning algorithm";
      break;
    case Algorithm::GapClustering:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a gap clustering algorithm";
      break;
    case Algorithm::agglomerativeHierarchical:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a Simple Merge Clustering algorithm";
      break;
    case Algorithm::DBSCAN:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a DBSCAN algorithm";
      break;
    case Algorithm::PVR:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a PVR algorithm";
      break;
    case Algorithm::adaptiveVertexReconstruction:
      edm::LogInfo("VertexProducerDownstream")
          << "VertexProducerDownstream::Finding vertices using an AdaptiveVertexReconstruction algorithm";
      break;
    case Algorithm::HPV:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a Highest Pt Vertex algorithm";
      break;
    case Algorithm::Kmeans:
      edm::LogInfo("VertexProducerDownstream") << "VertexProducerDownstream::Finding vertices using a kmeans algorithm";
      break;
  }
...
void VertexProducerDownstream::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const {
  ...
  if (settings_.debug() > 1) {
    edm::LogInfo("VertexProducerDownstream") << "produce::Processing " << l1TracksHandle->size() << " tracks";
  }
  for (const auto& track : *l1TracksHandle) {
    ...
    if (l1track.pt() >= settings_.vx_TrackMinPt()) {
      ...
      if (settings_.debug() > 2) {
        edm::LogInfo("VertexProducerDownstream") << "produce::Removing track with too low of a pt (" << l1track.pt() << ")\n"
                                       << "         word = " << l1track.getTTTrackPtr()->getTrackWord().to_string(2);
      }
    }
  }
  
  // for (size_t i = 0; i < l1SelectedTracksHandle->size(); i++) {
  //   // const auto& track = l1TracksHandle->at(i);
  //   // auto j = TTTrackRef(l1SelectedTrackHandle, i);
  //   auto j = TTTrackPtr(l1SelectedTracksHandle, i);
  //   auto k = L1Track(j);
  ...
  //   if (processSimulatedTracks_ && kinSel(track) && nPSStubsSel(track) && chi2Sel(track)) {
    ...
    // edm::Ptr<TTTrackType> ptr(track);
    // auto l1track = L1Track(TTTrackRef(*track));
    // auto l1track = L1Track(TTTrackPtr(track));
  // }
  // for (const auto& track : l1TracksHandle->ptrs()) {
  //   auto l1track = L1Track(track);
  //   std::cout << "old track pt: " << l1track.pt() << std::endl;;
  //   // edm::LogInfo("VertexProducerDownstream") << "old track pt: " << l1track.pt();
  ...
  // }
  if (settings_.debug() > 1) {
    edm::LogInfo("VertexProducerDownstream") << "produce::Processing " << l1Tracks.size() << " tracks after minimum pt cut of"
...
DEFINE_FWK_MODULE(VertexProducerDownstream);
