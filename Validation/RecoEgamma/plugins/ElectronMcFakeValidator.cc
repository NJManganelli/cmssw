...
ElectronMcFakeValidator::ElectronMcFakeValidator(const edm::ParameterSet &conf) : ElectronDqmAnalyzerBase(conf) {
  ...
  h1_matchingObjectEta_Extended = nullptr;
  h1_matchingObjectAbsEta = nullptr;
  h1_matchingObjectAbsEta_Extended = nullptr;
  ...
  h1_ele_matchingObjectEta_Extended_matched = nullptr;
  h1_ele_matchingObjectAbsEta_matched = nullptr;
  h1_ele_matchingObjectAbsEta_Extended_matched = nullptr;
  ...
  h1_scl_full5x5_sigmaIetaIeta_Extended_ = nullptr;
...
void ElectronMcFakeValidator::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const &, edm::EventSetup const &) {
  ...
  h1_matchingObjectEta_Extended =
      bookH1withSumw2(iBooker, "matchingObject_eta_Extended", matchingObjectType + " #eta", eta_nbin_extended, eta_min_extended, eta_max_extended, "#eta");
  ...
  h1_matchingObjectAbsEta_Extended =
      bookH1withSumw2(iBooker, "matchingObject_abseta_Extended", matchingObjectType + " |#eta|", eta_nbin_extended / 2, 0., eta_max_extended);
  h1_matchingObjectP =
  ...
  h1_ele_matchingObjectEta_Extended_matched =
      bookH1withSumw2(iBooker, "matchingObjectEta_Extended_matched", htitle + "#eta", eta_nbin_extended, eta_min_extended, eta_max_extended);
  ...
  h1_ele_matchingObjectAbsEta_Extended_matched =
      bookH1withSumw2(iBooker, "matchingObjectAbsEta_Extended_matched", htitle + "|#eta|", eta_nbin_extended / 2, 0., eta_max_extended);
  ...
  h1_scl_full5x5_sigmaIetaIeta_ = bookH1withSumw2(iBooker,
                                                  ...
                                                  "Events",
                                                  "ELE_LOGY E1 P");
  h1_scl_full5x5_sigmaIetaIeta_Extended_ = bookH1withSumw2(iBooker,
  ...
  h2_ele_EoPVsEta_Extended =
      bookH2(iBooker, "EoPVsEta_Extended", "ele E/P_{vertex} vs eta", eta2D_nbin_extended, eta_min_extended, eta_max_extended, eop2D_nbin, 0., eopmaxsht);
  h2_ele_EoPVsPhi =
  ...
  h1_ele_provenance_Extended = bookH1withSumw2(iBooker, "provenance_Extended", "ele provenance_Extended", 5, -2., 3.);
...
void ElectronMcFakeValidator::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (moIter = genJets->begin(); moIter != genJets->end(); ++moIter) {
    ...
    if (moIter->energy() / cosh(moIter->eta()) > maxPt_ || std::abs(moIter->eta()) > maxAbsEtaExtended_) {
      continue;
    }

    if (std::abs(moIter->eta()) < maxAbsEta_) {
      h1_matchingObjectEta->Fill(moIter->eta());
      h1_matchingObjectAbsEta->Fill(std::abs(moIter->eta()));
      h1_matchingObjectAbsEta_Extended->Fill(std::abs(moIter->eta()));
    ...
    if (std::abs(moIter->eta()) < maxAbsEtaExtended_) {
      h1_matchingObjectEta_Extended->Fill(moIter->eta());
      h1_matchingObjectAbsEta_Extended->Fill(std::abs(moIter->eta()));
    }
    ...
    if (okGsfFound) {
      ...
      isEEflag = bestGsfElectron.isEE() && (std::abs(moIter->eta()) < maxAbsEta_);
      ...
      if (isEEextendedflag) {  // Extended flag
        if (!isEEflag) {
          h1_ele_matchingObjectEta_Extended_matched->Fill(moIter->eta());
          h1_ele_matchingObjectAbsEta_matched->Fill(std::abs(moIter->eta()));
          h1_ele_matchingObjectAbsEta_Extended_matched->Fill(std::abs(moIter->eta()));
          ...
          h1_scl_full5x5_sigmaIetaIeta_Extended_->Fill(bestGsfElectron.full5x5_sigmaIetaIeta());
          h1_ele_HoE_Extended->Fill(bestGsfElectron.hadronicOverEm());
          ...
          h1_ele_neutralHadronRelativeIso_Extended->Fill(bestGsfElectron.pfIsolationVariables().sumNeutralHadronEt /
                                                bestGsfElectron.pt());
          h1_ele_photonRelativeIso_Extended->Fill(bestGsfElectron.pfIsolationVariables().sumPhotonEt / bestGsfElectron.pt());
