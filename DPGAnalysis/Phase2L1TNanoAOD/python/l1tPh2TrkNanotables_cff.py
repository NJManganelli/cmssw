...
#### GTT displaced vertices (track pairs from extended tracks).
#### NOTE (20->17 backport): central CMSSW_17 already ships a L1DispVertex MAIN
#### table (dispVtxTable in l1tPh2Nanotables_cff, scheduled by addPh2L1Objects)
#### carrying score/isReal/d_T/R_T/cos_T/del_Z/x/y/z/openingAngle/parentPt. To
#### avoid a "multiple main tables for L1DispVertex" collision this fork table is
#### emitted as an EXTENSION (extension=True) contributing ONLY the additional
#### track-index / trajectory columns not present upstream
#### (firstIndexTrk/secondIndexTrk index the L1TExtTrack table). Requires the
#### central main table to be present (it is, via addPh2L1Objects in the unified tier).
l1tDisplacedVertexTable = cms.EDProducer(
    ...
    doc = cms.string("GTT displaced vertices from extended track pairs (track-index extension)"),
    cut = cms.string(""),
    singleton = cms.bool(False), # the number of entries is variable
    extension = cms.bool(True),
    variables = cms.PSet(
